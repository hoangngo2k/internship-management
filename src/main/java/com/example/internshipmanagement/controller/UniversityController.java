...
@RestController
@RequestMapping("/home-admin/universities")
public class UniversityController {
    ...
    public ModelAndView getAllMentor(Model model,
        ...
        Page<University> universityPage = service.getAll(pageable, searchForm.getKeyword(), page, size, sort, field);
    ...
    public RedirectView saveMentor(@ModelAttribute("university") UniversityDto universityDto) {
        ...
        universityDto.setCreateId(Math.toIntExact(userDto.getId()));
        universityDto.setModifiedId(Math.toIntExact(userDto.getId()));
        service.save(universityDto);
        return new RedirectView("/home-admin/universities/");
    }
    ...
    public RedirectView updateMentor(@PathVariable Long id, @Valid UniversityDto universityDto,
        ...
        if (result.hasErrors()) {
            ...
            return new RedirectView("/home-admin/universities/update-form/" + id);
        }
        Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
        UserDto userDto = (UserDto) authentication.getPrincipal();
        userDto = userService.getUserByUsername(userDto.getUsername());
